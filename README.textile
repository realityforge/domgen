...
20.         t.reference(:User, :name => :PostedBy, :inverse_relationship_type => :none)
...
31.         t.reference(:Post, :immutable => true, :inverse_relationship_type => :has_many)
32.         t.reference(:User, :name => :PostedBy, :inverse_relationship_type => :none)
...
-  @t.reference(:User, :name => :PostedBy, :inverse_relationship_type => :none)@  (Line 20) := A reference to another object, in this case, back to @User@. The @:inverse_relationship => :none@ indicated that the inverse relationship _back_ to @Post@ will _not_ be created on the @User@ class. @:name => :PostedBy@ gives a custom name to both the model field and the database column.

- @t.sql.index[:PostedAt]@ (Line 26) := Creates an SQL index on that column.

- @t.reference(:Post, :immutable => true, :inverse_relationship_type => :has_many)@ (Line 31) := A relationship from @Comment@ to @Post@. The @:inverse_relationship => :has_many@ creates a collection model association from @Post@ to @Comment@, i.e _Posts have many comments_ .
...
* Sometimes you want to enable a family of generators with a single define_generator statement. Maybe use patterns ala :jpa* or similar. 