  ...
  module Generator
    ...
    def self.define_mssql_templates
      [
        Template.new(Sql::FACETS,
                     :data_module,
                     "#{Sql::TEMPLATE_DIRECTORY}/mssql_ddl.sql.erb",
                     '#{data_module.name}/schema.sql',
                     [::Domgen::Ruby::MssqlHelper],
                     'data_module.sql?'),
        Template.new(Sql::FACETS,
                     :data_module,
                     "#{Sql::TEMPLATE_DIRECTORY}/mssql_finalize.sql.erb",
    ...
    def self.define_pgsql_templates
      [
        Template.new(Sql::FACETS,
                     :data_module,
                     "#{Sql::TEMPLATE_DIRECTORY}/pgsql_ddl.sql.erb",
                     '#{data_module.name}/schema.sql',
                     [],
                     'data_module.sql?'),
        Template.new(Sql::FACETS,
                     :data_module,
                     "#{Sql::TEMPLATE_DIRECTORY}/pgsql_finalize.sql.erb",
                     '#{data_module.name}/finalize/schema_finalize.sql',
                     [],
                     'data_module.sql?'),
      ]
