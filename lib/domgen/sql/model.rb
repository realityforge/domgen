...
module Domgen
  module Sql
    ...
    class Index < Domgen.ParentedElement(:table)
      ...
      attr_writer :covering

      def covering?
        @covering.nil? ? false : @covering
      end
    ...
    facet.enhance(Entity) do
      ...
      def post_verify
        self.indexes.select { |index| index.covering? }.each do |index|
          ...
          entity_attribute_names = self.entity.attributes.collect{|attribute| attribute.name.to_s}.sort
          index.include_attribute_names = entity_attribute_names - index_attribute_names
        end
...
SQL
        ...
        entity.attributes.select { |a| a.allows_length? && a.has_non_default_min_length? }.each do |a|
          ...
          sql = (a.nullable? ? "#{self.dialect.quote(a.sql.column_name)} IS NULL OR " : '') + self.dialect.min_length_constraint(a.sql.column_name, a.min_length)
          constraint(constraint_name, :standard => true, :sql => sql) unless constraint_by_name(constraint_name)
        end
